...
export default function NotificationsPage() {
  ...
  const counts = useMemo(() => ({
    ...
    site_visit: notifications.filter((n) => n.type === 'site_visit').length,
  }), [notifications]);
      ...
      {permission !== 'granted' && 'Notification' in window && (
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-amber-50 border border-amber-200">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-sm font-bold transition flex-shrink-0"
          >
            Enable
          </button>
        </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        {(Object.keys(FILTER_LABELS) as FilterType[]).map((key) => (
          ...
          >
            {FILTER_LABELS[key]}
            {counts[key] > 0 && (
              <span className={`ml-1.5 text-[10px] font-bold ${filter === key ? 'text-[#1E293B]/70' : 'text-slate-500'}`}>
                {counts[key]}
              </span>
            )}
          </button>
