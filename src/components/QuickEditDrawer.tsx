...
export default function QuickEditDrawer({ lead, open, projects = [], onClose, onSaved }: QuickEditDrawerProps) {
  ...
  return (
    <>
      ...
      />
      {/* Drawer */}
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-sm bg-white border-l border-slate-200 shadow-2xl z-50 flex flex-col animate-[slideInRight_.2s_ease-out]">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 flex-shrink-0">
          <div className="min-w-0">
            <h2 className="text-lg font-bold text-[#0F172A] truncate">{lead.client_name}</h2>
            <p className="text-sm text-slate-500">{lead.phone}</p>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition flex-shrink-0"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium border transition ${
                      active
                        ? `${colors.bg} ${colors.text} ${colors.border} ring-2 ring-offset-1 ring-offset-white ring-current/20`
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 rounded-xl surface-dark text-slate-900 focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20 outline-none transition"
            >
          ...
          {/* Notes */}
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Quick Notes</label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              rows={4}
              className="w-full px-3.5 py-2.5 rounded-xl surface-dark text-slate-900 focus:border-[#D4AF37] focus:ring-2 focus:ring-[#D4AF37]/20 outline-none transition resize-none"
